...
int main(void)
{
    ...
    InitAudioDevice(); 
    Music colonna = LoadMusicStream("./audio/colonnas.mp3");        
    ...
    Texture2D * mario_alive = handleTextures("mario");          
    Texture2D * mario_jump = handleTextures("mario_jump");          
    Texture2D * mario_dead = handleTextures("mario_dead");          
    Texture2D * mush = handleTextures("mushroom");         
    Texture2D * bg = handleTextures("bg");         
    Texture2D * grasss = handleTextures("grass");         
    Texture2D * tree1 = handleTextures("tree1");         
    Texture2D * tree2 = handleTextures("tree2");         
    Texture2D * tree3 = handleTextures("tree3");         
    Texture2D * tree4 = handleTextures("tree4");         
    Texture2D * tree5 = handleTextures("tree5");         
    Texture2D * tree6 = handleTextures("tree6");         
    ...
    while (!WindowShouldClose())
    {
        if(death!=1&&stop!=1){
            ...
            //COLLISIONS
            if( CheckCollisionRecs(mario.collider, ground)&&mario.vel.y > 0)
            {
            ...
            for(int i=0;i<obstacoles_size;i++){
                ...
                {   
                    if(obstacoles[i].enemy==true) {
                        if(mario.vel.y <= 0){PlaySound(colpito);death=1;break;}
                        else {
                            obstacoles[i].collider=(Rectangle){0,0,0,0};
                            obstacoles[i].pos=(Vector2){0,0};
        ...
        BeginDrawing();
            ...
            for(int i=0;i<obstacoles_size;i++){
                ...
                else if(obstacoles[i].collider.height!=0&&obstacoles[i].collider.width!=0)DrawTexture(*mush ,obstacoles[i].pos.x, obstacoles[i].pos.y, WHITE);
            }
